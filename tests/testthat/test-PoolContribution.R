test_that("Checking the codon pool contribution", {

  data(subset_mRNA_data)
  data(metadata)

  tTEscanR_obj <- Create_tTEscanR_Object(counts = subset_mRNA_data, assay = "mRNA")

  # CASE 1
  # base - all inputs in the object
  expect_error(ExamineCodonPoolContribution(object = tTEscanR_obj, species = "hg38", verbose = FALSE)) # codon usage not present in object

  tTEscanR_obj <- ComputeCodonUsage(object = tTEscanR_obj, species = "hg38", filter = "canonical", additional.metrics = FALSE)

  expect_error(ExamineCodonPoolContribution(object = tTEscanR_obj, species = "hg38", verbose = FALSE)) # metadata and corr_factor not present in object

  tTEscanR_obj <- Update_tTEscanR_Object(object = tTEscanR_obj, meta.data = list(metadata, "tissue"),
                                         meta.data.ids = list("ConditionsLabels", "CorrectionFactor"))

  expect_no_error(ExamineCodonPoolContribution(object = tTEscanR_obj, species = "hg38", verbose = FALSE))

})
