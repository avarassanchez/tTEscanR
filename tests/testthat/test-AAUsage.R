test_that("The different modes to compute the amino acid usage", {
  ...
  tTEobject <- ComputeCodonUsage(object = tTEobject, codon_freq = NULL, species = "hg38", filter = "canonical", reduce = 1000, additional.metrics = FALSE, verbose = FALSE)
  ...
  tTEobject <- Create_tTEscanR_Object(counts = subset_mRNA_data, assay = "mRNA", meta.data = list(metadata, "tissue"),
