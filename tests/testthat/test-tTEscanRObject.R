test_that("Update_tTEscanR_Object adds new counts and metadata correctly", {
  ...
  expect_error(updated_obj <- Update_tTEscanR_Object(object = obj, counts = new_counts, assay = "mRNA"), "overwrite = TRUE")
  expect_no_error(updated_obj <- Update_tTEscanR_Object(object = obj, counts = new_counts, assay = "mRNA", overwrite = TRUE))
  updated_obj <- Update_tTEscanR_Object(object = obj, counts = new_counts, assay = "mRNA", overwrite = TRUE)
...
test_that("Update_tTEscanR_Object handles overwrite correctly", {
  ...
  expect_error(Update_tTEscanR_Object(object = obj, counts = matrix(5:8, nrow = 2), assay = "mRNA"), "overwrite = TRUE")

  # CASE 2: Overwrite with overwrite = TRUE (should succeed)
  new_RNA_counts <- matrix(9:12, nrow = 2)
  updated_obj <- Update_tTEscanR_Object(object = obj, counts = new_RNA_counts, assay = "mRNA", overwrite = TRUE)
...
test_that("The tTEscanR object is correctly generated", {
  ...
  tTEobject <- Create_tTEscanR_Object(counts = mRNA_data_test, assay = "mRNA")
  ...
  expect_no_error(Create_tTEscanR_Object(counts = mRNA_data_test, assay = "mRNA"))
  expect_no_error(Create_tTEscanR_Object(counts = list(mRNA = mRNA_data_test)))
  expect_no_error(Create_tTEscanR_Object(counts = list(mRNA_data_test), assay = list("mRNA")))

  # CASE 2: error - counts has a wrong data format
  expect_error(Create_tTEscanR_Object(counts = mRNA_data_test))
  expect_error(Create_tTEscanR_Object(counts = c(1, 2, 3, 4, 5), assay = "mRNA"))
  expect_error(Create_tTEscanR_Object(counts = c(mRNA_data_test), assay = "mRNA"))

  # CASE 3: error - wrong usage of the assay parameter
  expect_error(Create_tTEscanR_Object(counts = mRNA_data_test, assay = mRNA))
  expect_error(Create_tTEscanR_Object(counts = mRNA_data_test, assay = c("mRNA", "mRNA_2")))

  # CASE 4: no error - incorporation of metadata
  expect_no_error(Create_tTEscanR_Object(counts = mRNA_data_test, assay = "mRNA", meta.data = cell_types, meta.data.ids = "cell.types"))
  expect_no_error(Create_tTEscanR_Object(counts = list(mRNA = mRNA_data_test), meta.data = cell_types, meta.data.ids = "cell.types"))
  expect_no_error(Create_tTEscanR_Object(counts = list(mRNA_data_test), assay = list("mRNA"), meta.data = cell_types, meta.data.ids = "cell.types"))
  expect_error(Create_tTEscanR_Object(counts = mRNA_data_test, meta.data = list(cell_types), meta.data.ids = list("cell.types")))
  expect_no_error(Create_tTEscanR_Object(counts = list(mRNA_data_test), assay = "mRNA", meta.data = cell_types, meta.data.ids = "cell.types"))

  # CASE 5: error - repeated ids
  expect_no_error(Create_tTEscanR_Object(counts = list(mRNA = mRNA_data_test), assay = list("mRNA"), meta.data = cell_types, meta.data.ids = "cell.types")) # The name from the assay will be used
  expect_no_error(Create_tTEscanR_Object(counts = list(mRNA_data_test), assay = list("mRNA"), meta.data = list(cell_types = cell_types), meta.data.ids = list("cell.types")))
  expect_error(Create_tTEscanR_Object(counts = list(mRNA_data_test, mRNA_data_test), assay = list("mRNA", "mRNA"), meta.data = cell_types, meta.data.ids = "cell.types"))
})
...
test_that("The tTEscanR object is correctly updated", {
  ...
  expect_no_error(Update_tTEscanR_Object(object = tTEobject, counts = default_tTEscanR_tRNA_data, assay = "tRNA")) # just counts
  expect_no_error(Update_tTEscanR_Object(object = tTEobject, meta.data = cell_types, meta.data.ids = "cell_types")) # just metadata
  expect_no_error(Update_tTEscanR_Object(object = tTEobject, counts = default_tTEscanR_tRNA_data, assay = "tRNA", meta.data = cell_types, meta.data.ids = "cell_types")) # counts and metadata

  # CASE 2: use of overwrite
  tTEobject <- Update_tTEscanR_Object(object = tTEobject, counts = default_tTEscanR_tRNA_data, assay = "tRNA")
  expect_s4_class(object = tTEobject, class = "tTEscanR_Object")
  ...
  expect_error(Update_tTEscanR_Object(object = tTEobject, counts = default_tTEscanR_tRNA_data, assay = "tRNA")) # tRNA already in tTEobject
  expect_no_error(Update_tTEscanR_Object(object = tTEobject, counts = default_tTEscanR_tRNA_data, assay = "tRNA", overwrite = TRUE))
  expect_error(Update_tTEscanR_Object(object = tTEobject, counts = list(default_tTEscanR_mRNA_data, default_tTEscanR_tRNA_data), assay = list("mRNA", "tRNA")))
  expect_no_error(Update_tTEscanR_Object(object = tTEobject, counts = list(default_tTEscanR_mRNA_data, default_tTEscanR_tRNA_data), assay = list("mRNA", "tRNA"), overwrite = TRUE))

  # CASE 3: different input formats (list or single)
  expect_no_error(Update_tTEscanR_Object(object = tTEobject, counts = list(default_tTEscanR_mRNA_data), assay = list("mRNA"), overwrite = TRUE))
  expect_no_error(Update_tTEscanR_Object(object = tTEobject, counts = list(default_tTEscanR_mRNA_data), assay = "mRNA", overwrite = TRUE))
  expect_no_error(Update_tTEscanR_Object(object = tTEobject, counts = default_tTEscanR_mRNA_data, assay = list("mRNA"), overwrite = TRUE))
  expect_error(Update_tTEscanR_Object(object = tTEobject, counts = c(default_tTEscanR_mRNA_data, default_tTEscanR_tRNA_data), assay = c("mRNA", "tRNA"), overwrite = TRUE)) # using vectors
  ...
  expect_error(Update_tTEscanR_Object(object = tTEobject, counts = default_tTEscanR_tRNA_data)) # no ids
  expect_error(Update_tTEscanR_Object(object = tTEobject, meta.data = default_tTEscanR_tRNA_data)) # no ids
...
test_that("The function to find sections in a tTEscanR object works", {
  ...
  tTEobject <- Create_tTEscanR_Object(counts = default_tTEscanR_mRNA_data, assay = "mRNA")
  tTEobject <- Update_tTEscanR_Object(object = tTEobject, counts = default_tTEscanR_tRNA_data, assay = "tRNA")
  tTEobject <- Update_tTEscanR_Object(object = tTEobject, meta.data = colnames(default_tTEscanR_mRNA_data), meta.data.ids = "cell.types")
  ...
  expect_no_error(IsIn_tTEscanR_Object(object = tTEobject, slot = "assays", section = "mRNA", update_assay = FALSE, overwrite = FALSE))
  expect_no_error(IsIn_tTEscanR_Object(object = tTEobject, slot = "assays", section = "tRNA", update_assay = FALSE, overwrite = FALSE))
  expect_no_error(IsIn_tTEscanR_Object(object = tTEobject, slot = "meta.data", section = "cell.types", update_assay = FALSE, overwrite = FALSE))
  ...
  expect_no_error(IsIn_tTEscanR_Object(object = tTEobject, slot = "assay", section = "mRNA")) # with the implementation of match.arg() it is considered correctly as assays
  ...
  expect_no_error(IsIn_tTEscanR_Object(object = tTEobject, slot = "meta", section = "cell.types"))
})
...
test_that("The metadata is correctly updated", {
  ...
  expect_no_error(Update_tTEscanR_Object(object = tTEobject, meta.data = list(ENSG = ENSG_gene_names_mRNA_data), meta.data.ids = "ENSG_gene_names_mRNA_2"))
  ...
  expect_no_error(Update_tTEscanR_Object(object = tTEobject, meta.data = list(ENSG_gene_names_mRNA_data), meta.data.ids = "ENSG_gene_names_mRNA", overwrite = TRUE))
  expect_no_error(Update_tTEscanR_Object(object = tTEobject, meta.data = ENSG_gene_names_mRNA_data, meta.data.ids = "ENSG_gene_names_mRNA", overwrite = TRUE))
  expect_no_error(Update_tTEscanR_Object(object = tTEobject, meta.data = list(ENSG_gene_names_mRNA_data), meta.data.ids = list("ENSG_gene_names_mRNA"), overwrite = TRUE))
})
