test_that("The different corr_method possibilities - inside ComputeCodonUsage()", {
  ...
  tTEobject <- Create_tTEscanR_Object(counts = default_tTEscanR_mRNA_data, assay = "mRNA", meta.data = list(default_tTEscanR_metadata, "tissue"), meta.data.ids = list("ConditionsLabels", "CorrectionFactor"))
...
test_that("The different corr_method possibilities - inside ComputeAdditionalMetrics_CodonUsage()", {
  ...
  tTEobject <- Create_tTEscanR_Object(counts = default_tTEscanR_mRNA_data, assay = "mRNA", meta.data = default_tTEscanR_metadata, meta.data.ids = "ConditionsLabels")
  ...
  expect_no_error(suppressWarnings(ComputeAdditionalMetrics_CodonUsage(codon_usage = tTEobject@assays$CodonUsage, codon_freq = codon_freq_table_canonical_hg38,
  ...
  expect_no_error(suppressWarnings(ComputeAdditionalMetrics_CodonUsage(codon_usage = tTEobject@assays$CodonUsage, codon_freq = codon_freq_table_canonical_hg38, metadata = tTEobject@meta.data$ConditionsLabels, corr_method = "kendall", batch = "tissue", verbose = FALSE)))

  # CASE 3: error - trying non-existent corr_method parameters
  expect_error(suppressWarnings(ComputeAdditionalMetrics_CodonUsage(codon_usage = tTEobject@assays$CodonUsage, codon_freq = codon_freq_table_canonical_hg38,
...
test_that("The mean usage across different features - ComputeMeanUsage()", {
  ...
  tTEobject <- Create_tTEscanR_Object(counts = list(default_tTEscanR_mRNA_data, default_tTEscanR_tRNA_data), assay = list("mRNA", "tRNA"), meta.data = list(default_tTEscanR_metadata, "tissue"), meta.data.ids = list("ConditionsLabels", "CorrectionFactor"))
  tTEobject <- ComputeCodonUsage(object = tTEobject, codon_freq = NULL, species = "hg38", additional_metrics = FALSE)
