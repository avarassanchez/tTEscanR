test_that("Check the codon frequency per gene function - ObtainCodonFreqPerGene()", {
  ...
  expect_error(ObtainCodonFreqPerGene(dataset_name = NULL, genes_file = NULL, transcripts = NULL, filter = "canonical",
                                     out_format = "external_gene_name", verbose = TRUE))
  # CASE 2
  # error - wrong genes_file
  expect_error(ObtainCodonFreqPerGene(dataset_name = NULL, genes_file = genes, transcripts = NULL, filter = "canonical",
                                      out_format = "external_gene_name", verbose = TRUE))
})
...
test_that("Check the codon composition - ExtractCodonComposition()", {
  ...
  # CASE 2
  # error - wrong sequences input
  expect_error(ExtractCodonComposition(sequences = ATGCTGCAT, verbose = FALSE)) # not a string
  expect_error(ExtractCodonComposition(sequences = "ATGCTGCATC", verbose = FALSE)) # not a multiple of 3
