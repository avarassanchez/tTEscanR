...
can be directly loaded to perform the analysis. A step-by-step explanation of 
...
tTEobject <- runPipeline(
    ...
    species = "hg38", batch = "tissue", sample_id = "conditions",
...
| `plotTEscore()` | Represents the tTE scores obtained from 
...
head(SummarizedExperiment::assay(tTEobject, "CodonUsage"))
```

```{r codon_usage_data_transform}
long_format_codon_usage <- transformFormat(
    data = SummarizedExperiment::assay(tTEobject, "CodonUsage"), 
    normalize = TRUE,
...
long_format_codon_usage <- long_format_codon_usage %>%
    tidyr::separate(
        .data$condition, into = c("tissue", "cell_type"), sep = "-"
    )
```
...
long_format_AA_demand <- transformFormat(
    data = SummarizedExperiment::assay(tTEobject, "AADemand"), 
    normalize = TRUE,
...
appearance |
| `filename` | Optional; Name for the saved plot |
...
spleen_indexes <- grep("Spleen", long_format_codon_usage$tissue, fixed = TRUE)
long_format_cu_spleen <- long_format_codon_usage[spleen_indexes, ]

lymphoid_indexes <- grep(
    "Lymphoid", long_format_cu_spleen$cell_type, fixed = TRUE
)
myeloid_indexes <- grep(
    "Myeloid", long_format_cu_spleen$cell_type, fixed = TRUE
)
...
spleen_indexes <- grep("Spleen", long_format_codon_usage$tissue, fixed = TRUE)
heart_indexes <- grep("Heart", long_format_codon_usage$tissue, fixed = TRUE)
...
condition and the overall mean. This visualization helps identify how codon, 
...
cerebellum_indexes <- grep("Cerebellum", colnames(codon_usage), fixed = TRUE)
cerebrum_indexes <- grep("Cerebrum", colnames(codon_usage), fixed = TRUE)
...
meta <- S4Vectors::metadata(tTEobject)
...
specific metrics' distribution across features or conditions. It supports 
...
```{r mean_AA_demand}
# Data subset: mean codon usage across cerebellum cell types
aa_demand <- SummarizedExperiment::assay(tTEobject, "AADemand")
cerebellum_indexes_AA <- grep("Cerebellum", colnames(aa_demand), fixed = TRUE)
cerebrum_indexes_AA <- grep("Cerebrum", colnames(aa_demand), fixed = TRUE)
...
```{r extract_metadata}
tTEobject_meta <- S4Vectors::metadata(tTEobject)[["tTE_results"]]
tTEresults_codon <- tTEobject_meta[["tTEresults_codon"]]
tTEresults_AA <- tTEobject_meta[["tTEresults_AA"]]

conditions_metadata <- as.data.frame(SummarizedExperiment::colData(tTEobject))
```
...
conditions_metadata$group[
    grep("Myeloid", conditions_metadata$conditions, fixed = TRUE)
] <- "myeloid"
conditions_metadata$group[
    grep("Lymphoid", conditions_metadata$conditions, fixed = TRUE)
] <- "lymphoid"
```
...
conditions_metadata$group[grep(
    "neuron", conditions_metadata$conditions, fixed = TRUE
)] <- "neurons"
conditions_metadata$group[grep(
    "ENS neuron", conditions_metadata$conditions, fixed = TRUE
)] <- "other"
```
