...
output: 
  BiocStyle::html_document:
    ...
    css: style.css
vignette: >
  %\VignetteIndexEntry{2. tRNA-Specific Preprocessing Module}
...
To illustrate the functionality of the tRNA specific preprocessing module we 
...
# Create peak names with "-" separator 
...
for (i in 1:ncol(counts)) {
    ...
    for (j in 1:nrow(counts)) {
        cnt <- counts[j, i]
        if (cnt > 0) {
            parts <- strsplit(rownames(counts)[j], "-", fixed = TRUE)[[1]]
            for (k in 1:cnt) {
