...
::: {.note}
The **tTEscanR object** is a centralized data structure (`MultiAssayExperiment`) 
that stores input matrices, metadata, and results, continuously updated to 
...
**tTEscanR** object with new data or metadata, use **`updateObject()`**. 
...
tTEobject <- updateObject(
    object = tTEobject, 
    params = list("matching_celltypes" = matching_celltypes)
)
```

Each component of a **tTEscanR** object can be accessed using the 
`SummarizedExperiment::asay()` or `S4Vectors::metadata()` functions that 
requires the object and the name of the slot that wants to be retrieved. 
...
reference matrix can be generated using **`getCodonFreq()`** or 
...
previously using **`getCodonFreq()`**), or (ii) loaded from the 
...
tTEobject <- updateObject(
    object = tTEobject, params = list("CorrectionFactor" = "tissue")
)
...
```{r extract_meta, message = FALSE, warning = FALSE}
tTEobject_meta <- S4Vectors::metadata(tTEobject)
```

```{r correlation_plot_mean, message = FALSE, warning = FALSE}
# Transforming the data
additional_metrics <- tTEobject_meta[["CodonUsage_AdditionalMetrics"]]
...
# Transforming the data
codon_pool_contr <- tTEobject_meta[["CodonPoolContribution_Results"]]
...
tTEobject_meta <- S4Vectors::metadata(tTEobject)
tTEresults <- tTEobject_meta[["tTE_results"]]
tTEresults_codon <- tTEresults[["tTEresults_codon"]]
tTEresults_AA <- tTEresults[["tTEresults_AA"]]
...
# mRNA <- SummarizedExperiment::assay(tTEobject, "mRNA")
# CodonUsage <- SummarizedExperiment::assay(tTEobject, "CodonUsage")
# tRNA <- SummarizedExperiment::assay(tTEobject, "tRNA")
# AnticodonUsage <- SummarizedExperiment::assay(tTEobject, "AnticodonUsage")

AA_results <- list(
    AADemand = SummarizedExperiment::assay(tTEobject, "AADemand"),
    AASupply = SummarizedExperiment::assay(tTEobject, "AASupply")
)
```
...
all_DESeq2_results <- runDEAnalysis(
    list_data = AA_results, metadata = default_tTEscanR_metadata, 
    heatmap = TRUE, dim_reduct = "PCA", numPC = 2, batch = "tissue",
    color_factor = "tissue", show_legend = "right", label_factor = "cell.type"
)

# Visualization of the plots
grid::grid.draw(all_DESeq2_results$plots$AADemand$heatmap) # Heatmap plot
all_DESeq2_results$plots$AADemand$exploratory$ElbowPlot # Elbow plot
all_DESeq2_results$plots$AADemand$exploratory$PC1_vs_PC2 # PCA plot
```
