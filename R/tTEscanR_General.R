...
#' exonic background, (ii) mean codon usage, and (iii) correlation between the
...
#' analysis to each assay in the \code{tTEscanR_Object}. Defaults to \code{TRUE}
...
RunPipeline <- function(
    mRNA_data, tRNA_data, metadata, batch = NULL, corr_method = "spearman",
    additional_metrics = TRUE, compute_significance = TRUE, codon_freq = NULL,
    species = NULL, genetic_code = "Standard", dim_reduct = NULL, reduce = 100,
    runDESeq = TRUE, color_factor = NULL, verbose = TRUE
) {
    if (verbose) {
        message(
    ...
    if (runDESeq && verbose) message("(E) Differential expression analysis\n")
...
RunDEpipeline <- function(object, dim_reduct, verbose) {
    ...
    available_assays <- getAssay(object)[names(getAssay(object)) %in%
        target_assays]
    ...
    DESeq2_results <- RunDEAnalysis(
        list_data = available_assays, metadata = meta, batch = batch,
