...
#' conditions in the matrices of \code{list_data}. There has to be one column
...
#' reference for the corrections. If not specified, the 1st factor that appears
...
#' plots based on \code{fc_threshold} and \code{padj_threshold}. Defaults to
#' \code{TRUE}.
...
#' \code{metadata} to group by color the data points. Used if \code{dim_reduct}
#' specified.
#' @param shape_factor Optional; name of the categorical variable in
#' \code{metadata} to group by shape the data points. Used if \code{dim_reduct}
#' specified.
...
#' based on the enabled parameters: (i) exploratory plots (heatmap and/or PCA),
#' (ii) targeted plot (volcano), and (iii) size-corrected \code{data}.
#' @export
#'
#' @examples
...
RunDEAnalysis <- function(
    list_data, metadata, condition = NULL, batch = NULL, reference = NULL,
    reduce = 100, dim_reduct = NULL, color_factor = NULL, heatmap = TRUE,
    shape_factor = NULL, label_factor = NULL, highlight_median = FALSE,
    numPC = 2, color_palette = NULL, fc_threshold = 1, padj_threshold = 0.05,
    label_significant = TRUE, verbose = TRUE, show_legend = "none"
) {
    # Compute results
...
#'
#' @param list_data A \code{list} of matrices with features (i.e. genes, codons,
#' anticodons or amino acids) as rows and samples or conditions as columns. The
#' list can be named or unnamed.
#' @param metadata A \code{data.frame} with the data associated to the
#' conditions in the matrices of \code{list_data}. There has to be one column
#' with the same labels as the column names.
#' @param condition Optional; a factor based on \code{metadata} columns to
...
#'     metadata = default_tTEscanR_metadata, batch = "tissue"
#' )
ComputeDEResults <- function(
    list_data, metadata, condition = NULL, batch = NULL, reference = NULL,
    reduce = 100, padj_threshold = 0.05, verbose = TRUE
) {
...
#' to extract.
#' @param dim_reduct Either \code{"PCA"}, \code{"UMAP"} or \code{"tSNE"} to
#' specify the dimensionality reduction approach to be executed. Defaults to
#' \code{"PCA"}.
#' @param color_factor Optional; name of the categorical variable in
#' \code{metadata} to group by color the data points. Used if \code{dim_reduct}
#' specified.
...
#' \code{NULL}.
#' @param fc_threshold Numeric; fold change threshold used for highlighting
#' significant features in the volcano plot (if \code{targets} is specified).
#' Defaults to 1.
#' @param padj_threshold Numeric; p-value threshold used for highlighting
#' significant features in the volcano plot. Defaults to 0.05.
#' @param label_significant Logical; if \code{TRUE} displays the axis of the
...
#' )
PlotDEResults <- function(
    DE_results_list, dataset_name = NULL, heatmap = TRUE, dim_reduct = NULL,
    numPC = 2, condition = NULL, verbose = TRUE, color_factor = condition,
    shape_factor = NULL, label_factor = NULL, highlight_median = FALSE,
    scale_pca = FALSE, color_palette = NULL, fc_threshold = 1,
    padj_threshold = 0.05, label_significant = TRUE, show_legend = "none"
) {
...
CheckSpecificParameters <- function(color, shape, label, meta) {
    ...
    if (length(missing_factors) > 0) {
        stop("The following factors are not found in the metadata: ",
            missing_factors, collapse = ","
        )
    }
}
