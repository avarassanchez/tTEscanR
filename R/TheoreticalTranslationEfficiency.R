...
#' information
#' representing the translation efficiency table for the matching conditions in
#' the mRNA and tRNA data.
#' @export
#'
#' @examples
#' data(
...
#' )
Compute_tTE <- function(
    object, level = c("codon", "aa", "both"), genetic_code = "Standard",
...
HelperComputetTE <- function(
    results, level, verbose, object, meta, batch, corr, sig, genetic_code
) {
...
AssignMapLevel <- function(level, tRNA_exp, data_mRNA, genetic_code) {
    ...
    tRNAsAnticodons <- vapply(strsplit(tRNA_names, "-"), "[[", 3,
...
ComputePvalues <- function(
    tTE_conditions, data_mRNA, tRNA_exp_filtered, N_perm, map_factor,
    corr_method, tTE_scores
) {
    ...
    for (i in seq_len(n_cond)) { # Iterate over conditions present in the data
        ...
        for (j in seq_len(N_perm)) { # Iterates over number of iterations
            ...
            null_correlations[j] <- stats::cor( # Fills matrix with tTE score
                agg_usage, mRNA_vec,method = corr_method
            )
        }
...
ComputeStatisticalSignificance <- function(
    level, tTE_scores, data_mRNA, data_tRNA, tRNA_exp, corr_method,
    genetic_code, N_perm = 1000, verbose
) {
...
FilterMatrix <- function(
    data_mRNA, data_tRNA, level = c("codon", "aa"), verbose
) {
    level <- match.arg(level)
