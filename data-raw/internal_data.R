...
final_matrix_genetic_code <- gen_codes %>%
    rowwise() %>%
    mutate(AA_List = list(strsplit(AA_String, "")[[1]])) %>%
    ungroup() %>%
    unnest(AA_List) %>%
...
###
genes <- c(
    "ADCY9", "YIPF1", "TMEM106A", "GFPT2", "GOT1L1", "CCHCR1", "CAMKK2",
    "ROGDI", "CEP89", "HSDL2", "NOC3L", "COL5A2", "ANO4", "CCRL2"
)

genes_translated <- c(
    "ENSG00000162104", "ENSG00000058799", "ENSG00000184988", "ENSG00000131459",
    "ENSG00000169154", "ENSG00000204536", "ENSG00000110931", "ENSG00000067836",
    "ENSG00000121289", "ENSG00000119471", "ENSG00000173145", "ENSG00000204262",
    "ENSG00000151572", "ENSG00000121797"
)

cell_types <- c(
    "Adrenal-Adrenocortical cells", "Adrenal-Chromaffin cells",
    "Adrenal-CSH1_CSH2 positive cells", "Adrenal-Erythroblasts",
    "Adrenal-Lymphoid cells", "Adrenal-Megakaryocytes", "Adrenal-Myeloid cells",
    "Adrenal-Schwann cells", "Adrenal-SLC26A4_PAEP positive cells",
    "Adrenal-Stromal cells", "Adrenal-Sympathoblasts",
    "Adrenal-Vascular endothelial cells", "Kidney-Erythroblasts",
    "Kidney-Lymphoid cells", "Kidney-Megakaryocytes",
    "Kidney-Mesangial cells", "Kidney-Metanephric cells",
    "Kidney-Myeloid cells", "Kidney-Stromal cells", "Kidney-Ureteric bud cells"
)

mRNA_values <- c(
    3186, 939, 1, 6, 3, 5, 5, 18, 0, 436, 145, 391, 1, 9, 2, 4752, 7770, 20, 125, 583,
...
mRNA_data_test_translated <- matrix(
    mRNA_values,
    nrow = 14,
    byrow = TRUE,
    dimnames = list(genes_translated, cell_types)
)
...
# "#ffffbf",
aa_colors <- c(
    Phe = "#7f3b08", Ile = "#b35806", Met = "#e08214", Val = "#fdb863", Ser = "#fee0b6",
...
gradual_groups_35 <- c(
    "#e08214", "#35978f", "#de77ae", "#7fbc41", "#9970ab", "#d6604d", "#4393c3",
